...
if __name__ == "__main__":
    ...
    Hqs = hamiltonian(static, [], basis=basis, check_herm=False, check_pcon=False, check_symm=False)
    Hqs.toarray()
    
    H = Oper(static, ["s"])
    H.show_oper()
    
    H = sum( xx(i, (i+1)%L) + yy(i, (i+1)%L) + zz(i, (i+1)%L) for i in range(L-1)) + z(1)
    H.get_matrix(basis)
    H.toarray()
    H.show()
    ...
    H = Oper(static,["f"])
    H.show()
    H = sum(pm_f(i,(i+1)%L) + mp_f(i,(i+1)%L) + nn_f(i,(i+1)%L) for i in range(L-1)) + z_f(1)
    H.show()
    ...
    H = Oper(static,["b"])
    H.show()
    H = sum(pm_b(i,(i+1)%L) + mp_b(i,(i+1)%L) + nn_b(i,(i+1)%L) for i in range(L-1)) + n_b(1)
    H.show()
    ...
    H.get_LinearOperator(sb)
    s2 = H.action_on(s1)
    show_state(sb, s2)
    
    H.get_matrix(sb)
    s2 = H.matrix.toarray() @ s1
    show_state(sb, s2)
    ...
    print("===========================")
    print("Example 17")
    H = 2*pm_b(0,1) + 2*mp_b(0,1) + 4*nn_b(0,1) + 4*n_b(1) + 7*bdag(1) + 4*b(0)  # type: Oper
    H.show_oper()
    ...
    H.get_matrix(basis=mb, check_pcon=False)
    s4 = H.matrix.toarray() @ s3
    show_state(mb, s4)
    ...
    print("Example 19")
    site_num = 3
    H = sum(mp(i,i+1)+pm(i+1,i)+zz(i,i+1) 
    ...
    H.show_oper()
    # """第二种方法"""
    H = sum(pm_b(i,i+1) + pm_b(i+1,i) for i in range(mode_num-1))\
        + pm_b(mode_num-1,0) + pm_b(0,mode_num-1)
    H.show_oper()
...
#     print("===========================")
    ...
    H.get_matrix(s_par=1, b_par=[1,4],pauli=-1)
    H.get_eig()
    H.show_eig(vrtlist=True)

    """ Example 24 """
    print("===========================")
    print("Example 24")
    site_num = 10
    H = sum(mp(i,i+1)+pm(i,i+1)+zz(i,i+1) for i in range(site_num-1)) \
        + mp(0,site_num-1) \
        + pm(site_num-1,0) \
        + zz(0,site_num-1)
    H.show_oper()
    ...
    print("Example 26")
    site_num = 3
    H = sum(pm_f(i,i+1)+pm_f(i+1,i)+zz_f(i,i+1) for i in range(site_num-1)) \
        + pm_f(0,site_num-1) \
        + pm_f(site_num-1,0) \
        + zz_f(0,site_num-1)
    H.show_oper()
    ...
    H.get_matrix(sb)  # 这个是在自然基上写的 我这里的自然基是指按 ...fdag_i...fdag_j |0> 中 i<j 排列的
    H.show_matrix()
    H.get_eig()
    H.show_eig()
    H.show_eigvalue()
