...
from pynotes.张量网络笔记.BasicTools import choose_device, combine_dicts
...
def split_time_series(data, length, device=None, dtype=tc.float32):
    """
    利用 length 长度的时序数据预测第 length+1 位置的数据
    :param data: 一维时序数据
    :param length: 样本长度
    :param device: 计算设备
    :param dtype: 数据精度
    :return: N * length 维的样本矩阵，N 维的标签向量
    """
